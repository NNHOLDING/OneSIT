    ...
    if datos and len(datos) > 1:
        ...
        if "fecha" not in df.columns or "certificador" not in df.columns or "ruta" not in df.columns:
            ...
            if "empresa" in df_filtrado.columns:
                ...
                # Limpiar nombre de empresa si incluye certificador u otros datos
                df_filtrado["empresa_limpia"] = df_filtrado["empresa"].str.split(" - ").str[0]

                cert_por_empresa = df_filtrado["empresa_limpia"].value_counts()

                fig, ax = plt.subplots(figsize=(6, 6))
                ax.pie(
                    cert_por_empresa,
                    labels=cert_por_empresa.index,
                    autopct="%1.1f%%",
                    startangle=90,
